...
./mvnw clean spring-boot:build-image \
  --projects "$MOVIE_API_APP_NAME" \
  -DskipTests="$SKIP_TESTS" \
  -Dspring-boot.build-image.imageName="$MOVIE_API_DOCKER_IMAGE_NAME"

./mvnw clean spring-boot:build-image \
  --projects "$MOVIE_UI_APP_NAME" \
  -DskipTests="$SKIP_TESTS" \
  -Dspring-boot.build-image.imageName="$MOVIE_UI_DOCKER_IMAGE_NAME"
